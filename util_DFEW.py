...
def train_one_epoch(model, optimizer, data_loader, device, epoch):
    ...
    for step, data in enumerate(data_loader):
        ...
        # print(images1.shape,target.shape)
...
def evaluate(model, data_loader, device, epoch,dataset):
    ...
    draw_confusion_matrix(label_true=y_gt,  # y_gt=[0,5,1,6,3,...]
                          ...
                          normlize=True,
                          title="Confusion Matrix on AffectNet",
                          pdf_save_path=dataset+str(epoch)+"_Conf"+'.svg',
                          dpi=600)
    Visualization(feat, lab, epoch,dataset+str(epoch)+"_feat"+'.svg',600 )
...
def draw_confusion_matrix(label_true, label_pred, label_name, normlize, title="Confusion Matrix", pdf_save_path=None,
    ...
    for i in range(label_name.__len__()):
        for j in range(label_name.__len__()):
            ...
            # print('cm[i,j]',cm[i,j])
            # value = cm[i, j]
            value = round(cm.T[i,j], 4)
            value = value * 100
            value = round(value,2)
            # value = float(format('%.2f' % cm[i, j]))
            # print('value',value)

            plt.text(i, j, value, fontsize = 12,verticalalignment='center', horizontalalignment='center', color=color)
