...
class FasterNet(nn.Module):
    def __init__(self,
        ...
        # self.stages_list1 = nn.Sequential(*stages_list[:1])
        # self.stages_list2 = nn.Sequential(*stages_list[1:3])
        # self.stages_list3 = nn.Sequential(*stages_list[3:5])
        # self.stages_list4 = nn.Sequential(*stages_list[5:])
    ...
    def forward_cls(self, x):
        ...
        # print('x2:', x.shape)
        # b_l, c, h, w = x.shape
        # x = x.reshape((b_l // 16, 16, c * h * w))
        #
        # x = self.avgpool_pre_head(x)  # B C 1 1
        # print('poolong',x.shape)
...
def fasternet_s(pretrained=True,**kwargs):
    ...
    if pretrained:
        ...
        checkpoint = torch.load('./runs/02_best_model.pth')
        model.load_state_dict(checkpoint,False)
        num_fr = model.head.in_features
        model.head = nn.Linear(num_fr, 7)
    return model
